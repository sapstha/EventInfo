...
<html>
    ...
    <%
            ...
            try {
                ...
                if (request.getParameter("Action").equalsIgnoreCase("save")) {
                    try {
                        ...
                        sql = "insert into edufair VALUES(null,'" + event + "','" + date + "','" + venue + "','" + organizer + "')";
                        //out.print(sql);
                        int row = db.st.executeUpdate(sql);
                        out.print("<script>alert('Data Saved');window.location.assign('updateedu.jsp');</script>");
                    } catch (Exception e) {
                    ...
                    try {
                        ...
                        venue = request.getParameter("venue");
                        organizer = request.getParameter("organizer");
                        sql = "update edufair set event='" + event + "', date='" + date + "', venue='" + venue + "',organizer='" + organizer +"' where id='" + id + "'";
                        //out.print(sql);
                        int row = db.st.executeUpdate(sql);
                        out.print("<script>alert('Updated');window.location.assign('updateedu.jsp');</script>");
                    } catch (Exception e) {
                        out.print(e.getMessage());
                    }
                }
                    ...
                    try {
                     id = request.getParameter("id");
                        btnValue = "Update";
                       sql = "select * from edufair where id='" + id + "'";
                       out.print(sql);
        ...
        <div id="container">
            ...
            <div id="contentevent">
                <div id="contentabc1">
             <h1>Update Educational fairs</h1>
        <a href="updateedu.jsp">Back</a>
        <form method="get">
